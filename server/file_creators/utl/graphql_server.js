...
function createSubQuery(field, data, database) {
  ...
  if (database.includes('MySQL') || database.includes('PostgreSQL')) {
    if (database.includes('MySQL')) query += `getConnection`
    if (database.includes('PostgreSQL')) query += `connect`
...
//   let type = `const poolQuery = require('../db/${databaseType}_pool');
...
//       let firstPrimaryKey = '';
//       console.log('pkey', firstPrimaryKey)
...
//           if (field.primaryKey) {
//               query += `,\n\t\tget${table.type}By${toTitleCase(field.name)}: (parent, { ${field.name} }) => {
//           const sql = \`SELECT * FROM ${table.type} WHERE ${field.name} = \${${field.name}}\`;
